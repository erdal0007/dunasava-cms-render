...
export default function AdminSectors() {
  ...
  const createMut = trpc.cms.sectorCreate.useMutation({ onSuccess: () => { utils.cms.sectorList.invalidate(); setEditing(null); } });
  const updateMut = trpc.cms.sectorUpdate.useMutation({ onSuccess: () => { utils.cms.sectorList.invalidate(); setEditing(null); } });
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setEditing({ ...emptyForm })} className="flex items-center gap-2 bg-[#4A7C59] text-white px-4 py-2 rounded-lg text-sm hover:bg-[#5A8C69]"><Plus size={16} /> Yeni Sektör</button>
      </div>
